...
def merge_new_data(existing_df, new_df):
    ...
    for key, new_row in new_df.iterrows():
        if key in existing_df.index:
            ...
            # Update Actual if existing is empty and new has a value
            existing_actual = str(existing_df.at[key, "Actual"]).strip() if pd.notna(existing_df.at[key, "Actual"]) else ""
            new_actual = str(new_row["Actual"]).strip() if pd.notna(new_row["Actual"]) else ""
            if not existing_actual and new_actual:
                existing_df.at[key, "Actual"] = new_actual
                row_updated = True

            # Update ActualDir if existing is empty and new has a value
            existing_actual_dir = str(existing_df.at[key, "ActualDir"]).strip() if pd.notna(existing_df.at[key, "ActualDir"]) else ""
            new_actual_dir = str(new_row["ActualDir"]).strip() if pd.notna(new_row["ActualDir"]) else ""
            if not existing_actual_dir and new_actual_dir:
                existing_df.at[key, "ActualDir"] = new_actual_dir
                row_updated = True

            # Update Detail if existing is empty and new has a value
